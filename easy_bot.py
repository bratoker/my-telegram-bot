...
async def holidays(update, context):
    now = datetime.datetime.now()
...
async def kosti_start(update, context):
...
async def kosti_guess(update, context):
    try:
...
async def suyefa_start(update, context):
...
async def suyefa_choice(update, context):
    mapping = {
...
async def mines_count(update, context):
...
async def launge_start(update, context):
    await update.message.reply_text(
        "🌍 *Переводчик RU ↔ EN*\n\n"
...
async def launge_translate(update, context):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "Отправь ещё или /start для меню.",
            parse_mode="Markdown"
        )
...
async def kalck_start(update, context):
    await update.message.reply_text(
...
async def kalck_compute(update, context):
...
async def ai_start(update, context):
